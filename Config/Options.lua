    ...
    args = {
      ...
      helpers = {
        ...
        args = {
          ...
          intro = {
            type = "description",
            name = "Pre-pull + situational reminder badges (food, flask, battle/guardian elixir, weapon stone, pet food, plus conditional ones vs Demon / Undead bosses). Buffs are matched by spell ID, so every rank counts. A missing buff shows a greyed badge with its name in the band; one that's about to run out shows in colour with a countdown. Scroll badges say which scroll (AGI / STR) and carry a YOU or PET strip. With Click to apply on, a badge whose consumable is in your bags pulses and uses it on click. Buffs that are comfortably up stay invisible. Unlock frames (General tab) to move the panel.",
            order = 1,
            fontSize = "medium",
          },
          parseMode = {
          ...
          helpersExpiringThreshold = {
            ...
            desc = "Surface a badge with a countdown when a tracked buff has less than this many seconds left — the window in which you'd top it up before a pull. 0 disables the early warning, so badges only appear once a buff is fully missing. Up to 30 minutes, so a freshly applied scroll can be watched counting down.",
            order = 4,
            min = 0, max = 1800, step = 15,
            width = "full",
            get = function() return Nock.db.profile.helpersExpiringThreshold or 180 end,
            set = function(_, v) Nock.db.profile.helpersExpiringThreshold = v end,
          },
          helpersClickToApply = {
            type = "toggle",
            name = "Click to apply",
            desc = "Every badge with a matching consumable in your bags becomes a click target that uses it — press only, out of combat. Scroll badges apply the scroll they name (on you or on your pet), stone badges apply to your main hand. The gold pulse and the pointer mark the clickable ones.",
            order = 4.1,
            width = "full",
            get = function() return Nock.db.profile.helpersClickToApply ~= false end,
            set = function(_, v) visualsSet(_, "helpersClickToApply", v) end,
          },
          helpersHeadline = {
            type = "toggle",
            name = "Caption under the row",
            desc = "A muted PRE-PULL caption under the icons, with a CLICK TO APPLY hint while any badge is clickable. Off leaves the bare icon strip.",
            order = 4.2,
            width = "full",
            get = function() return Nock.db.profile.helpersHeadline ~= false end,
            set = function(_, v) visualsSet(_, "helpersHeadline", v) end,
          },
          ...
          consumeHeader = { type = "header", name = "Eating / drinking pill", order = 6 },
          consumeBannerEnabled = {
            type = "toggle",
            name = "Show while eating or drinking",
            desc = "A small centre-screen capsule with the food / drink icon, EATING or DRINKING, and the seconds left, up for as long as the aura is on you, wherever you are. Flashes WELL FED in green the moment the buff lands so you know you can stand up.",
            order = 6.1,
            width = "full",
            get = function() return Nock.db.profile.consumeBannerEnabled ~= false end,
            set = function(_, v) visualsSet(_, "consumeBannerEnabled", v) end,
          },
          consumeBannerSound = {
            type = "toggle",
            name = "Chime on WELL FED",
            desc = "Play the ready-check chime when Well Fed lands.",
            order = 6.3,
            width = "full",
            disabled = function() return Nock.db.profile.consumeBannerEnabled == false end,
            get = function() return Nock.db.profile.consumeBannerSound == true end,
            set = function(_, v) visualsSet(_, "consumeBannerSound", v) end,
          },
          consumeBannerSize = {
            type = "range",
            name = "Pill icon size",
            desc = "Edge of the food / drink icon in the pill; the text scales with it.",
            order = 6.4,
            min = 20, max = 64, step = 1,
            disabled = function() return Nock.db.profile.consumeBannerEnabled == false end,
            get = function() return Nock.db.profile.consumeBannerSize or 32 end,
            set = function(_, v) visualsSet(_, "consumeBannerSize", v) end,
          },
          consumeBannerResetPos = {
            type = "execute",
            name = "Reset pill position",
            desc = "Return the eating / drinking pill to its default spot above the boss alert.",
            order = 6.5,
            func = function()
              Nock.db.profile.consumeBannerPosition = false
              Nock:SendMessage("NOCK_POSITION_RESET")
            end,
          },
          layoutHeader2 = { type = "header", name = "Panel", order = 7 },
          helpersResetPos = {
            type = "execute",
            name = "Reset position",
